...
RUN apt-get update && \
    apt-get install -y zip unzip curl bzip2 python-dev build-essential git libssl1.1 libssl-dev
...
RUN apt-get install -y software-properties-common debconf-utils && \
    ...
    apt-get install -y -y openjdk-8-jdk
ENV JAVA_HOME=/usr/lib/jvm/java-8-openjdk-amd64

# Download and install Anaconda Python
ADD https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh /tmp/Miniconda3-latest-Linux-x86_64.sh
RUN bash /tmp/Miniconda3-latest-Linux-x86_64.sh -b -p /root/anaconda
ENV PATH="/root/anaconda/bin:$PATH"

#
# Install git, clone repo, install Python dependencies
#
RUN git clone https://github.com/martindanielj/Agile_Data_Code_2
WORKDIR /root/Agile_Data_Code_2
ENV PROJECT_HOME=/root/Agile_Data_Code_2
...
#
ADD https://archive.apache.org/dist/hadoop/common/hadoop-3.0.1/hadoop-3.0.1.tar.gz /tmp/hadoop-3.0.1.tar.gz
...
#
ADD https://archive.apache.org/dist/spark/spark-2.4.0/spark-2.4.0-bin-without-hadoop.tgz /tmp/spark-2.4.0-bin-without-hadoop.tgz
...
# Have to set spark.io.compression.codec in Spark local mode, give 3GB RAM
RUN cp /root/spark/conf/spark-defaults.conf.template /root/spark/conf/spark-defaults.conf && \
    ...
    echo "spark.driver.memory 3g" >> /root/spark/conf/spark-defaults.conf
...
ADD https://repo1.maven.org/maven2/org/mongodb/mongo-java-driver/3.4.0/mongo-java-driver-3.4.0.jar /tmp/mongo-java-driver-3.4.0.jar
...
#
ADD https://archive.apache.org/dist/kafka/0.10.2.2/kafka_2.11-0.10.2.2.tgz /tmp/kafka_2.11-0.10.2.2.tgz
RUN mkdir -p /root/kafka && \
    tar -xvzf /tmp/kafka_2.11-0.10.2.2.tgz -C kafka --strip-components=1 && \
    rm -f /tmp/kafka_2.11-0.10.2.2.tgz
...
#
WORKDIR /root
ADD https://mirrors.sonic.net/apache/zeppelin/zeppelin-0.9.0/zeppelin-0.9.0-bin-all.tgz /tmp/zeppelin-0.9.0-bin-all.tgz
